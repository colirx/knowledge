...
1. 重启 Linux，检查 selinux 和 swap 分区情况：
...
下面主要做的内容就是：在三台服务器中安装 Docker（18.06.0）、kubeadm（1.17.4）、kubelet（1.17.4）、kubectl（1.17.4）。
...
# 安装指定版本的 docker-ce，必须指定 --setopt=obsoletes=0，否则 yum 会安装更高的版本
yum -y install --setopt=obsoletes=0 docker-ce-18.06.3.ce-3.el7

# 添加一个配置文件，docker 在默认情况下使用的 Cgroup Driver 为 cgroupfs，而 kubernetes 推荐使用 systemd 来替代 cgroupfs，并且切换一下国内的仓库镜像
mkdir /etc/docker
...
```shell
# 切换为国内镜像源，编辑文件 /etc/yum.repos.d/kubernetes.repo，添加如下配置
[kubernetes]
name=Kubernetes
...
# 配置 kubelet 的 cgroup，编辑 /etc/sysconfig/kubelet（之前的内容都删掉），添加如下配置（需要改变 cgroupfs 为 systemd，并且改变代理模式为 ipvs，之前说过 ipvs 性能高）。
KUBELET_CGROUP_ARGS="--cgroup-driver=systemd"
KUBE_PROXY_MODE="ipvs"

# 设置 kubelet 开机自动启动（这个我们不用启动，因为后面集群启动会自动启动这个东西）
systemctl enable kubelet
```

**安装 kubernetes 组件**

首先我们一定要有一个清晰的认知：搭建 kubernetes 集群其实就是安装 kubernetes 组件。
